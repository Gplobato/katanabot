# bot.py - Katana ELITE7 com Evolution API
import os
import re
import requests
...
EVOLUTION_INSTANCE = os.environ.get("EVOLUTION_INSTANCE", "katanabot")
...
def normalize_phone(raw):
    ...
    # Evolution API precisa do @s.whatsapp.net
...
def send_via_evolution(phone, message):
    ...
    try:
        ...
        print(f"📋 Resposta Evolution: {r.text[:500]}")
...
def webhook():
    ...
    try:
        ...
        if "data" in incoming and isinstance(incoming["data"], dict):
            ...
            # Pega o remetente
            phone = data.get("key", {}).get("remoteJid") or data.get("remoteJid")
        ...
        # Fallback genérico
        if not phone:
            phone = incoming.get("remoteJid") or incoming.get("from") or incoming.get("phone")
        
        if not message:
            if isinstance(incoming.get("message"), dict):
                message = incoming["message"].get("conversation")
            else:
